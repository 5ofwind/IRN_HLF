...
def test(test_loader, filter_net, test_name, out):
    ...
    for image_num, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            LR = torch.cat((LR, LR2), dim=1)
            LR_d = torch.cat((LR_d, LR_d2), dim=1)
            LR_s = torch.cat((LR_s, LR_s2), dim=1)
            #print(LR.shape)

            prediction, out_d, out_s = filter_net(LR, LR_d, LR_s)
            #print(prediction.shape[1]/3)
            shape_prediction=int(prediction.shape[1]/2)
            #prediction=(prediction[:,0:shape_prediction,:,:,:]+flip(prediction[:,shape_prediction:shape_prediction*2,:,:,:],1)+prediction[:,shape_prediction*2:shape_prediction*3,:,:,:])/3   ###123
            prediction=(prediction[:,0:shape_prediction,:,:,:]+flip(prediction[:,shape_prediction:shape_prediction*2,:,:,:],1))/2   ###12
            #prediction=(flip(prediction[:,shape_prediction:shape_prediction*2,:,:,:],1)+prediction[:,shape_prediction*2:shape_prediction*3,:,:,:])/2   ###23            
            #prediction=prediction[:,0:shape_prediction,:,:,:]   ###1                 
            #prediction=(prediction[:,shape_prediction*2:shape_prediction*3,:,:,:]
#+flip(prediction[:,shape_prediction*3:shape_prediction*4,:,:,:],1))/2 ###34
            '''
            LR=flip(LR,1)
            LR_d=flip(LR_d,1)
            LR_s=flip(LR_s,1)
...
def save_img(prediction,test_name,image_num, att):
    if att == True:
        save_dir = './results/IRN_2stages/' + test_name#os.path.join(opt.image_out, systime)    
