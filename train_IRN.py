...
#rsdn.parameters()
    ...
    optimizer = optim.Adam(rsdn.parameters(), lr = opt.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=opt.weight_decay) 
    if opt.stepsize > 0:
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs+1):
        ...
        alter=train(train_loader, rsdn, opt.scale, criterion, optimizer, epoch, use_gpu, ttttt, alter) #fed data into network
...
def train(train_loader, rsdn, scale, criterion, optimizer, epoch, use_gpu, ttttt, alter):
    train_mode = True
    epoch_loss = 0
    rsdn.train()
...
#10000or100
        #print(iteration)
        #if (((iteration) % 10000 == 0) and ((epoch==1 and iteration==0)==False)):
        if ((iteration) % 10000 == 0):            
            ...
            print("===> Epoch[{}]({}/{}): Loss: {:.4f} || Timer: {:.4f} sec.".format(epoch, iteration, len(train_loader), average_loss/10000, total_time))
            average_loss=0
            total_time=0
            ...
            for scene_name in scene_list:
                test_set = get_test_set('/home/will/Documents/RRN-master/RRN/datasets/Vid4', opt.scale, scene_name)
                ...
                #print('===> DataLoading Finished')
                PSNR, SSIM, out = test(test_loader, rsdn, opt.scale, scene_name)
                PSNR_avg += PSNR
                SSIM_avg += SSIM
                count += 1
...
#prediction[:,shape_prediction*2:shape_prediction*3,:,:,:])/(B*T)
        ...
        out_d=(flip(out_d[:,shape_prediction:shape_prediction*2,:,:,:],1)+out_d[:,shape_prediction*2:shape_prediction*3,:,:,:])/2
        out_s=(flip(out_s[:,shape_prediction:shape_prediction*2,:,:,:],1)+out_s[:,shape_prediction*2:shape_prediction*3,:,:,:])/2
        loss=1/3*(criterion(prediction_final, target)/(B*T)+criterion(out_d, target2)/(B*T)+criterion(out_s, target3)/(B*T))#1/3*
#+0.1*loss4
        #loss=criterion(prediction_final, target)/(B*T)
        loss.backward()

        #print(torch.norm(rsdn.parameters(),2))
        #torch.nn.utils.clip_grad_norm(rsdn.parameters(),10)
        #optimizer2.step()
        optimizer.step()
...
def test(test_loader, rsdn, scale, test_name):
    train_mode = False
    rsdn.eval()
    count = 0
    PSNR = 0
